use crate::constants::{CHAR_HEIGHT_WITH_PADDING, CHAR_WIDTH, CONTENT_AREA_WIDTH, WINDOW_HEIGHT, WINDOW_PADDING, WINDOW_WIDTH};
    ...
    pub fn cascading_style(&mut self, declarations: Vec<Declaration>) {
        for declaration in declarations {
            match declaration.property.as_str() {
                ...
                "color" => {
                    if let ComponentValue::Ident(value) = &declaration.value {
                        // Color::from_name(&value).unwrap_or_else(|_| Color::white());
                    ...
                    if let ComponentValue::HashToken(color_code) = &declaration.value {
                        let color =
                            Color::from_code(&color_code).unwrap_or_else(|_| Color::black());
                        self.style.set_background_color(color);
                    }
                }
                "display" => {
                    if let ComponentValue::Ident(value) = &declaration.value {
                        let display_type = DisplayType::from_str(&value)
                            .unwrap_or_else(|_| DisplayType::DisplayNone);
    ...
    pub fn compute_size(&mut self, parent_size: LayoutSize) {
        ...
        match self.kind() {
            LayoutObjectKind::Block => {
                ...
                let mut height = parent_size.height;
...
fn split_text(line: String, char_width: i64) -> Vec<String> {
    ...
    if line.len() as i64 * char_width > (WINDOW_WIDTH + WINDOW_HEIGHT) {
        let s = line.split_at(find_index_for_line_break(line.clone(), ((WINDOW_WIDTH - WINDOW_PADDING) / char_width) as usize));
