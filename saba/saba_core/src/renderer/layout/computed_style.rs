...
impl ComputedStyle {
    ...
    pub fn font_size(&self) -> FontSize {
        self.font_size.expect("failed to access CSS property: font_size")
    }

    pub fn text_decoration(&self) -> TextDecoration {
        self.text_decoration.expect("failed to access CSS property: text_decoration")
    }
...
impl Color {
    ...
    pub fn from_code(code: &str) -> Result<Self, Error> {
        ...
        let name = match code {
            ...
            _ => {
                return Err(Error::UnexpectedInput(format!("color code {:?} is not supported yet", code)));
            }
        };
...
impl DisplayType {
    ...
    pub fn from_str(s: &str) -> Result<Self, Error> {
        match s {
            ...
            _ => Err(Error::UnexpectedInput(format!("display {:?} is not supported yet", s))),
        }
    }
}
